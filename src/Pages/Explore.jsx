...
const FamousArtwork = () => {
  ...
  useEffect(() => {
    const fetchArtworks = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`https://api.artic.edu/api/v1/artworks?page=${page}&limit=12&fields=id,title,artist_display,image_id`);
        const { data } = res.data;
        
        // Prepare array of promises to fetch IIIF URLs for each artwork
        const iiifUrlPromises = data.map(artwork => {
          if (artwork.image_id) {
            return axios.get(`https://www.artic.edu/iiif/2/${artwork.image_id}/full/843,/0/default.jpg`)
              .then(res => ({
                ...artwork,
    ...
    <>
          ...
          {artworks.map((artwork, index) => (
            <div key={index} className="masonry-item">
              ...
              <div className="masonry-item-content p-4">
                ...
                <p className="text-sm text-gray-600 mb-4">{artwork.artist_display}</p>
                <a href={`https://www.artic.edu/artworks/${artwork.id}`} target="_blank" rel="noopener noreferrer" className="text-gray-900 underline">View Details</a>
              </div>
            </div>
          ))}
        </Masonry>
      ...
      {hasMore && !loading && (
        <div className="flex justify-center my-4">
          <button className="bg-gray-900 text-white px-4 py-2 rounded-md" onClick={loadMore}>Load More</button>
        </div>
      )}
    </>
  );
